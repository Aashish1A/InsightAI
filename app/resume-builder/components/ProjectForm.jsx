      ...
      <div className="space-y-4 mt-6">
        {data.map((project, index) => (
          <div key={index} className="p-4 border border-gray-200 rounded-lg space-y-3" >
            ...
            <input value={project.link || ""} onChange={(e) => updateProject(index, "link", e.target.value)} type="text" placeholder="Project Link" className=" px-3 py-2 text-sm rounded-lg w-full" />
              <textarea value={project.description || ""} onChange={(e) => updateProject(index, "description", e.target.value) } type="text" rows={4} placeholder="Describe your project..." className=" px-3 py-2 text-sm rounded-lg resize-none w-full" />
          </div>
        ))}
      </div>
    </div>
