  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2">
        <label>
          {data.image ? (
            <Image src={ typeof data.image === "string" ? data.image : URL.createObjectURL(data.image)} alt="user-image" width={64} height={64} unoptimized className="w-16 h-16 rounded-full object-cover mt-5 ring ring-slate-300 hover:opacity-80" />
          ) : (
      ...
      {fields.map((field) => {
        ...
        return (
          <div key={field.key} className="space-y-1 mt-5">
            ...
            <input className="mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors text-sm" placeholder={`Enter your ${field.label.toLowerCase()}`} required={field.requried} type={field.type} value={data[field.key] || ""} onChange={(e) => handleChange(field.key, e.target.value)} />
          </div>
        );
      })}
    </div>
