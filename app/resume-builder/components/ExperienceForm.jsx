...
export default function ExperienceForm({ data, onChange }) {
  ...
  const generateDescription = async (index) => {
    ...
    try {
      const { data } = await axios.post("/api/ai/enhance-job-desc", {
        userContent: prompt,
      });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={addExperience} className="flex items-center gap-2 px-3 py-2 text-sm bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors" >
            ...
            <div key={index} className="p-4 border border-gray-200 rounded-lg space-y-3" >
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button className="flex items-center gap-1 px-2 py-1 text-sm bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors disabled:opacity-50" onClick={() => generateDescription(index)} disabled={ generatingIndex === index || !experience.position || !experience.company } >
