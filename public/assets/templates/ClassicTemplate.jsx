...
export default function ClassicTemplate({ data, accentColor }) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-7 bg-white text-gray-800 leading-relaxed">
      {/* Header */}
      <header className="text-center mb-1 pb-2 border-b-2" style={{ borderColor: accentColor }} >
        ...
        <div className="flex flex-wrap justify-center gap-1 text-sm text-gray-600">
          {data.personal_info?.email && (
            <div className="flex items-center gap-1">
              <a target="_blank" href={`mailto:${data.personal_info?.email}`} className="flex items-center gap-1" rel="noopener noreferrer" >
          ...
          {data.personal_info?.linkedin && (
            <div className="flex items-center gap-1">
              <a target="_blank" rel="noopener noreferrer" href={ data.personal_info?.linkedin.startsWith("https://") ? data.personal_info?.linkedin : `https://${data.personal_info?.linkedin}` } className="flex items-center gap-1" >
          ...
          {data.personal_info?.github && (
            <div className="flex items-center gap-1">
              <a target="_blank" rel="noopener noreferrer" href={ data.personal_info?.github.startsWith("https://") ? data.personal_info?.github : `https://${data.personal_info?.github}` } className="flex items-center gap-1" >
          ...
          {data.personal_info?.website && (
            <div className="flex items-center gap-1">
              <a target="_blank" rel="noopener noreferrer" href={ data.personal_info?.website.startsWith("https://") ? data.personal_info?.website : `https://${data.personal_info?.website}` } className="flex items-center gap-1" >
              ...
              <div key={index} className="flex justify-between items-start border-l-3 border-gray-300 pl-4" >
                <div>
                  <li className="font-semibold text-gray-800">
                    {proj.name}
                    {proj.link && (
                      <a href={proj.link} target="_blank" className="font-light cursor-pointer" style={{ color: accentColor }} >
                        {" "}
                        Link
                      </a>
                    )}
